...
namespace GEngine
{
	...
	public class JsonNode
	{
		...
		protected virtual List<JsonNode> AsArray()
		{
			throw new InvalidCastException();
		}
		protected virtual Dictionary<string, JsonNode> AsTable()
		{
			throw new InvalidCastException();
		}
		...
		public virtual void Add(JsonNode n)
		{
			throw new NotImplementedException();
		}
		...
		public static bool operator true(JsonNode node)
		{
			if (!node.IsAny())
				return false;
			if (node.IsNull())
		...
		public static bool operator false(JsonNode node)
		{
			if (!node.IsAny())
				return true;
			if (node.IsNull())
		...
		public static explicit operator List<JsonNode>(JsonNode node)
		{
			return node.AsArray();
		}
		public static explicit operator Dictionary<string, JsonNode>(JsonNode node)
		{
			return node.AsTable();
		}
		...
		public static implicit operator JsonNode(uint u)
		{
		...
		public static implicit operator JsonNode(List<JsonNode> list)
		{
			return new ArrayNode(list);
		}
		public static implicit operator JsonNode(Dictionary<string, JsonNode> table)
		{
		...
		public static JsonNode NewBool(bool b)
		{
			BoolNode node = new BoolNode(b);
			return node;
		}

		public static JsonNode NewInt(int i)
		{
			IntNode node = new IntNode(i);
			return node;
		}

		public static JsonNode NewUInt(uint u)
		{
			UIntNode node = new UIntNode(u);
			return node;
		}

		public static JsonNode NewNumber(double d)
		{
			NumberNode node = new NumberNode(d);
			return node;
		}

		public static JsonNode NewString(string s)
		{
		...
		public static JsonNode NewTable()
		{
		...
		protected class EmptyNode : JsonNode
		{
		...
		protected class NullNode : JsonNode
		{
		...
		protected class BoolNode : JsonNode
		{
			protected bool b;
		...
		protected class IntNode : JsonNode
		{
			protected int i;
		...
		protected class UIntNode : JsonNode
		{
			protected uint u;
		...
		protected class NumberNode : JsonNode
		{
			protected double d;
		...
		protected class StringNode : JsonNode
		{
			protected string s;
		...
		protected class ArrayNode : JsonNode
		{
			protected List<JsonNode> list;
			public ArrayNode()
			{
				list = new List<JsonNode>();
			}
			public ArrayNode(List<JsonNode> lst)
			{
				list = lst;
			}
			public override Kind GetKind()
			...
			protected override List<JsonNode> AsArray()
			{
				return list;
			}
			...
			public override void Add(JsonNode n)
			{
				list.Add(n);
			}
		...
		protected class TableNode : JsonNode
		{
			protected Dictionary<string, JsonNode> table;
			public TableNode()
			{
				table = new Dictionary<string, JsonNode>();
			}
			public TableNode(Dictionary<string, JsonNode> tb)
			{
				table = tb;
			}
			public override Kind GetKind()
			...
			protected override Dictionary<string, JsonNode> AsTable()
			{
				return table;
			}
			public override void Clear()
	...
	public struct JsonParser
	{
		...
		private static void _ArrayProc(JsonNode node, string key, JsonNode value)
		{
			((List<JsonNode>)node).Add(value);
		}

		private static void _TableProc(JsonNode node, string key, JsonNode value)
		{
			((Dictionary<string, JsonNode>)node)[key] = value;
		}

		private static Proc ArrayProc = _ArrayProc;
		private static Proc TableProc = _TableProc;

		private delegate void Proc(JsonNode node, string key, JsonNode value);
		...
		private bool Value(JsonNode value)
		{
            ...
            context.Error("< ERROR > JsonNode error!");
			return false;
		}
		...
		private bool ParseWord(Words word)
		{
			...
			case Words.ARRAY_BEGIN:
				{
					JsonNode newnode = JsonNode.NewArray();
					if (!Value(newnode))
						return false;
					NodeProc nodeproc;
			...
			case Words.OBJECT_BEGIN:
				{
					JsonNode newnode = JsonNode.NewTable();
					if (!Value(newnode))
						return false;
					NodeProc nodeproc;
					nodeproc.node = newnode;
			...
			case Words.INTEGER:
				{
					...
					if (use_double)
					{
						if (!Value(JsonNode.NewNumber(minus ? -d : d)))
							return false;
					}
					else
					{
						if (minus)
						{
							if (!Value(JsonNode.NewInt(-(int)n)))
								return false;
						}
						else
						{
							if (!Value(JsonNode.NewUInt(n)))
								return false;
						}
					}
				}
			...
			case Words.FLOAT:
				{
					...
					{
						if (!Value(JsonNode.NewNumber((minus ? -n : n) * POW10((dot < 0 ? 0 : -dot) + exp))))
							return false;
					}
				}
				break;
			case Words.NULL:
				{
					if (!Value(JsonNode.NewNull()))
						return false;
				}
				break;
			case Words.TRUE:
				{
					if (!Value(JsonNode.NewBool(true)))
						return false;
				}
				break;
			case Words.FALSE:
				{
					if (!Value(JsonNode.NewBool(false)))
						return false;
				}
				break;
			case Words.STRING:
				{
					...
					{
						if (!Value(JsonNode.NewString(str)))
							return false;
					}
				}
				break;
        ...
        string loadpath;
		public JsonNode Load(string input)
		{
			    return Load(Encoding.UTF8.GetBytes(input));
		}

		public JsonNode Load(byte[] input)
		{
			context.input = input;
			context.outset = 0;
			context.cursor = 0;
        ...
        public JsonNode Load(byte[] input, string path)
        {
            loadpath = path;
            return Load(input);
        }
	...
	public class JsonPrinter
	{
		...
		private Dictionary<JsonNode, bool> writed;
		public JsonPrinter()
		{
			writer = new Writer(false);
			writed = new Dictionary<JsonNode, bool>();
		}
		public JsonPrinter(bool pretty)//是否格式化保存
		{
			if (pretty)
			{
		...
		public JsonPrinter(bool pretty, bool escape)
		{
			if (pretty)
			{
		...
		private bool Write(JsonNode node)
		{
			switch (node.GetKind())
			{
			...
			case JsonNode.Kind.BOOLEAN:
				...
				return true;
			case JsonNode.Kind.INT:
				writer.WriteInt((int)node);
				return true;
			case JsonNode.Kind.UINT:
				writer.WriteUInt((uint)node);
				return true;
			case JsonNode.Kind.DOUBLE:
			...
			case JsonNode.Kind.ARRAY:
				{
					...
					List<JsonNode> list = (List<JsonNode>)node;
			...
			case JsonNode.Kind.TABLE:
				{
					if (writed.ContainsKey(node))
						return false;
					writed.Add(node, true);
					Dictionary<string, JsonNode> table = (Dictionary<string, JsonNode>)node;
					writer.WriteNewTable(table.Count);
					bool first = true;
					foreach (KeyValuePair<string, JsonNode> kv in table)
					{
						if (first)
						{
							first = false;
